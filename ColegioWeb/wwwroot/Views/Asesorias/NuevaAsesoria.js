...
function BuscarCurso(valor) {
    ...
    resumenFiltro.forEach(function (item) {
        $("#contenedor-cursos").append(
            `<div class="col mb-4">
                <div class="card h-100 card-curso" style="cursor: pointer" data-id="${item.idCurso}" data-text="${item.nombre}">
                    <div class="card-body">
...
function BuscarDocente(valor) {
    $("#contenedor-docente").html(""); // Limpiar la lista de docentes antes de agregar nuevos

    const resultadosFiltrados = Docentes.filter(element => (element.nombres + " " + element.apellidos).toLowerCase().includes(valor.toLowerCase()));
    console.log($("#contenedor-docente"));  // Verifica si el contenedor está presente
        ...
        $("#contenedor-docente").append(
            `<div class="col mb-4">
                <div class="card h-100 card-docente" style="cursor: pointer" data-id="${item.idDocente}" data-text="${acronimo} ${item.nombres} ${item.apellidos}">
                    <div class="card-body">
                        <h5 class="card-title"><i class="fa-solid fa-user-docente"></i> ${acronimo} ${item.nombres} ${item.apellidos}</h5>
                    </div>
                </div>
            </div>`
        );
...
function ObtenerDocentes() {
    ...
    fetch(`/Docente/Lista`, {
        ...
        if (responseJson.data.length > 0) {
            // Filtro por curso
            Docentes = responseJson.data.filter(element => element.curso.idCurso == IdCursoSeleccionado);
