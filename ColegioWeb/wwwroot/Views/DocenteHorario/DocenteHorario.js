let tablaData;
let idEditar = 0;
const controlador = "DocenteHorario";
const modal = "mdData";
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    tablaData = $('#tbData').DataTable({
        ...
        "columns": [
            {
                title: "Nro Documento", "data": "docente", width: "150px",render: function (data, type, row) {
    ...
    fetch(`/Docente/Lista`, {
        ...
        if (responseJson.data.length > 0) {
            $("#cboDocente").append($("<option>").val("").text(""));
            responseJson.data.forEach((item) => {
                $("#cboDocente").append($("<option>").val(item.idDocente).text(`${item.numeroDocumentoIdentidad} - ${item.nombres} ${item.apellidos}`));
            });
            $('#cboDocente').select2({
                theme: 'bootstrap-5',
...
$("#tbData tbody").on("click", ".btn-eliminar", function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch(`/${controlador}/Eliminar?Id=${data.idDocenteHorario}`, {
                method: "DELETE",
                headers: { 'Content-Type': 'application/json;charset=utf-8' }
            }).then(response => {
                return response.ok ? response.json() : Promise.reject(response);
